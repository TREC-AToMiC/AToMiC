...
class AtomicDataset(Dataset):
    ...
    def _get_info_dict(self, id_column):
        ...
        if self.cache_file_path.exists():
            ...
            id_index_dict = {row: index 
                for index, row in tqdm(enumerate(self[id_column]), total=len(self), desc='build id2row dict')
            }     
    ...
    def _get_split_dict(self, qrels, id_column):
        if self.cache_split_path.exists():
            ...
            for index, row in tqdm(enumerate(self[id_column]), total=len(self), desc='build split dict'):
                if row in train:
...
if __name__ == '__main__':
    dataset = AtomicDataset('TREC-AToMiC/AToMiC-Images-v0.2', id_column='image_id', qrel_name_or_path='TREC-AToMiC/AToMiC-Qrels-v0.2')
